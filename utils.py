...
def parse_command():
    ...
    parser.add_argument('--data', metavar='DATA', default='lidardepthv2',
                        choices=data_names,
                        help='dataset: ' + ' | '.join(data_names) + ' (default: lidardepthv2)')
...
def merge_into_row_with_gt(input, depth_input, depth_target, depth_pred, normal):
    rgb = 255 * np.transpose(np.squeeze(input.cpu().numpy()), (1,2,0)) # H, W, C
    ...
    img_merge = np.hstack([rgb, depth_input_col, depth_target_col, depth_pred_col, normal_cpu])
